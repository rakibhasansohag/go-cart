...
function id(kind: string, index: number) {
	const hex = createHash('sha256').update(`${PREFIX}:${kind}:${index}`).digest('hex').slice(0, 32);
...
const statusFixtures = [
	{ order: OrderStatus.Pending, item: ProductStatus.Pending, package: PackageStatus.PENDING, shipment: ShipmentStatus.AWAITING_RECEIPT },
	{ order: OrderStatus.Confirmed, item: ProductStatus.ReadyForShipment, package: PackageStatus.ACCEPTED, shipment: ShipmentStatus.AWAITING_RECEIPT },
	{ order: OrderStatus.Processing, item: ProductStatus.Processing, package: PackageStatus.PROCESSING, shipment: ShipmentStatus.AWAITING_RECEIPT },
	{ order: OrderStatus.Processing, item: ProductStatus.Processing, package: PackageStatus.READY_FOR_HANDOFF, shipment: ShipmentStatus.AWAITING_RECEIPT },
	{ order: OrderStatus.Processing, item: ProductStatus.Shipped, package: PackageStatus.HANDED_OFF, shipment: ShipmentStatus.RECEIVED_AT_HUB },
	{ order: OrderStatus.Delivered, item: ProductStatus.Delivered, package: PackageStatus.HANDED_OFF, shipment: ShipmentStatus.DELIVERED },
] as const;

const demoProductNames = [
	'Atlas Chronograph Watch', 'Harbor Knit Cotton Sweater', 'Aster Ceramic Travel Mug',
	'Northstar Wireless Headphones', 'Cedar & Stone Leather Wallet', 'Lumen 27-inch Monitor',
	'Cloudline Everyday Sneakers', 'Ridgeway Canvas Backpack', 'Solace Linen Throw Blanket',
	'Field Notes Hardcover Journal', 'Mistral Stainless Water Bottle', 'Cove Minimal Desk Lamp',
	'Juniper Botanical Candle', 'Summit Insulated Jacket', 'Willow Woven Crossbody Bag',
	'Ember Portable Bluetooth Speaker', 'Meadow Relaxed Fit T-shirt', 'Orbit Mechanical Keyboard',
	'Pinecrest Trail Running Shoes', 'Sable Wood Serving Board', 'Drift Ceramic Planter',
	'Verde Everyday Sunglasses', 'Beacon USB-C Charging Hub', 'Horizon Cotton Bath Towel',
	'Vale Softshell Travel Jacket',
] as const;

async function main() {
	const [, seller, customer] = await Promise.all([
		db.user.upsert({ where: { email: users.admin }, update: { name: 'Rakib Hasan Sohag', role: Role.ADMIN }, create: { name: 'Rakib Hasan Sohag', email: users.admin, role: Role.ADMIN, picture: 'https://i.pravatar.cc/160?img=12' } }),
		db.user.upsert({ where: { email: users.seller }, update: { name: 'Demo Seller', role: Role.SELLER }, create: { name: 'Demo Seller', email: users.seller, role: Role.SELLER, picture: 'https://i.pravatar.cc/160?img=33' } }),
	...
	const catalog = [] as Array<{ productId: string; variantId: string; sizeId: string; name: string; slug: string; variantSlug: string; sku: string; image: string; size: string; price: number }>;
	for (let index = 0; index < 25; index += 1) {
		...
		const imageUrls = [image, `https://picsum.photos/seed/gocart-demo-product-${index}-detail/640/640`, `https://picsum.photos/seed/gocart-demo-product-${index}-lifestyle/640/640`];
		await db.product.upsert({ where: { id: productId }, update: { name, description: `A practical ${name.toLowerCase()} selected for the GoCart demo catalog.` }, create: { id: productId, name, description: `A practical ${name.toLowerCase()} selected for the GoCart demo catalog.`, slug, brand: 'GoCart Demo', shippingFeeMethod: ShippingFeeMethod.ITEM, storeId: store.id, categoryId: category.id, subCategoryId: subCategory.id } });
		await db.productVariant.upsert({ where: { id: variantId }, update: { variantName: 'Standard', variantImage: image }, create: { id: variantId, variantName: 'Standard', variantDescription: `Standard ${name.toLowerCase()} variant`, variantImage: image, slug: variantSlug, sku, keywords: 'demo,fixture,test', weight: 1, productId } });
		await db.productVariantImage.deleteMany({ where: { productVariantId: variantId } });
		await db.productVariantImage.createMany({ data: imageUrls.map((url, order) => ({ id: id(`variant-image-${order}`, index), url, alt: `${name} — view ${order + 1}`, order, productVariantId: variantId })) });
		await db.size.upsert({ where: { id: sizeId }, update: { quantity: 100, price, discount: 0 }, create: { id: sizeId, size: 'Standard', quantity: 100, price, productVariantId: variantId } });
		catalog.push({ productId, variantId, sizeId, name, slug, variantSlug, sku, image, size: 'Standard', price });
	}

	const orderIds = Array.from({ length: COUNT }, (_, index) => id('order', index));
	const groupIds = Array.from({ length: COUNT }, (_, index) => id('group', index));
	...
	for (let index = 0; index < COUNT; index += 1) {
		...
		addresses.push({ id: addressId, firstName: 'Demo', lastName: `Customer ${index + 1}`, phone: '+15550001001', address1: `${index + 1} Demo Street`, address2: null, state: 'CA', city: 'San Francisco', zip_code: '94105', default: index === 0, userId: customer.id, countryId: country.id, createdAt, updatedAt: createdAt });
		orders.push({ id: orderIds[index], shippingFees: 0, subTotal: subtotal, total: subtotal, orderStatus: fixture.order, paymentStatus: PaymentStatus.Paid, paymentMethod: PaymentMethod.Stripe, shippingAddressId: addressId, userId: customer.id, createdAt, updatedAt: createdAt });
