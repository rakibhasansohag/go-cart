...
const ProductDetails: FC<ProductDetailsProps> = ({
	...
	const handleAIGenerate = async (generatedData: AIGeneratedProductData) => {
		...
		if (generatedData.brand) {
			...
			newAiFields.brand = true;
		}
		if (generatedData.variantName) {
			form.setValue('variantName', generatedData.variantName);
			newAiFields.variantName = true;
		}
		if (generatedData.variantDescription) {
			form.setValue('variantDescription', generatedData.variantDescription);
			newAiFields.variantDescription = true;
		}
		if (generatedData.sku) {
			form.setValue('sku', generatedData.sku);
			newAiFields.sku = true;
		}
		if (generatedData.weight !== undefined) {
			form.setValue('weight', generatedData.weight);
			newAiFields.weight = true;
		}

		// arrays
		if (generatedData.colors && generatedData.colors.length > 0) {
			setColors(generatedData.colors);
			newAiFields.colors = true;
		}
		if (generatedData.sizes && generatedData.sizes.length > 0) {
			setSizes(generatedData.sizes);
			newAiFields.sizes = true;
		}
		if (generatedData.product_specs && generatedData.product_specs.length > 0) {
			setProductSpecs(generatedData.product_specs);
			newAiFields.product_specs = true;
		}
		if (generatedData.variant_specs && generatedData.variant_specs.length > 0) {
			setVariantSpecs(generatedData.variant_specs);
			newAiFields.variant_specs = true;
		}
		if (generatedData.keywords && generatedData.keywords.length > 0) {
			setKeywords(generatedData.keywords);
			newAiFields.keywords = true;
		}
		if (generatedData.questions && generatedData.questions.length > 0) {
			setQuestions(generatedData.questions);
			newAiFields.questions = true;
		}

		setAiGeneratedFields(newAiFields);
										...
										<FormField
											...
											render={({ field }) => (
												<FormItem className='flex-1 w-full'>
													...
													{aiGeneratedFields.category && <AIBadge />}
													<FormMessage />
												</FormItem>
											)}
										/>
										<FormField
											...
											render={({ field }) => (
												<FormItem className='flex-1'>
													...
													{aiGeneratedFields.subCategory && <AIBadge />}
													<FormMessage />
												</FormItem>
											)}
										/>
