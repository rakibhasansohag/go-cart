...
async function runPaidPaymentSideEffects(input: {
  ...
  try {
    ...
    // PostgreSQL aborts a transaction after a unique-key conflict. A
    // concurrent verified event may have won the GoCoins idempotency key,
    // so inspect the committed state with the root client before logging a
    // real side-effect failure.
    const existingAward = isUniqueConstraintError(error)
      ? await db.loyaltyTransaction.findUnique({
