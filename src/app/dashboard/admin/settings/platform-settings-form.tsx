...
export default function PlatformSettingsForm({ initialCommissionPercent, initialPayoutHoldDays }: { initialCommissionPercent: number; initialPayoutHoldDays: number }) {
	const [commissionPercent, setCommissionPercent] = useState(String(initialCommissionPercent));
	const [payoutHoldDays, setPayoutHoldDays] = useState(String(initialPayoutHoldDays));
	...
	async function submit(event: FormEvent<HTMLFormElement>) {
		...
		try {
			...
			const data = await response.json() as { commissionPercent?: number; payoutHoldDays?: number; error?: string };
			if (!response.ok) throw new Error(data.error ?? 'Unable to save commission.');
			if (data.commissionPercent === undefined || data.payoutHoldDays === undefined) throw new Error('The server returned incomplete marketplace settings.');
			setCommissionPercent(String(data.commissionPercent));
			setPayoutHoldDays(String(data.payoutHoldDays));
			setMessage(data.payoutHoldDays === 0
	...
	return (
		<div className='mx-auto flex w-full max-w-3xl flex-col gap-6'>
			...
			<section className='rounded-xl border border-border p-5'>
				...
				<p className='mt-1 text-sm text-muted-foreground'>These settings apply to new settlement ledger entries. Existing settlements keep their original commission and eligibility date.</p>
				<form onSubmit={submit} className='mt-6 space-y-4'>
					...
					<label htmlFor='payout-hold-days' className='block text-sm font-medium'>Seller payout hold / return-risk window</label>
					<div className='flex max-w-xs items-center gap-2'>
						<input id='payout-hold-days' name='payoutHoldDays' type='number' min='0' max='365' step='1' required value={payoutHoldDays} onChange={(event) => setPayoutHoldDays(event.target.value)} className='w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary' />
						<span className='text-sm text-muted-foreground'>days</span>
					</div>
					<p className='text-xs text-muted-foreground'>After delivery evidence, seller funds wait this many days before entering a payday batch. The default is 7 days. Use 0 only for sandbox testing; it removes the payout protection buffer.</p>
					...
					<button type='submit' disabled={saving} className='rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50'>{saving ? 'Saving…' : 'Save marketplace settings'}</button>
				</form>
			</section>
		</div>
	);
