...
const { constructEventMock, getStripeClientMock, handleStripeEventMock } = vi.hoisted(() => ({
...
describe('Stripe webhook route', () => {
	...
	it('rejects an unconfigured webhook', async () => {
		...
		expect(await response.json()).toEqual({ error: 'Stripe webhook is not configured.' });
	});
	...
	it('returns a client error when Stripe signature verification fails', async () => {
		...
		expect(response.status).toBe(400);
		expect(await response.json()).toEqual({ error: 'No signatures found matching the expected signature for payload' });
	...
	it('passes the raw body to Stripe and reports duplicate events', async () => {
		...
		expect(constructEventMock).toHaveBeenCalledWith(rawBody, 'valid', 'whsec_test');
